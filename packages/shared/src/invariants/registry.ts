...
export const INVARIANTS: readonly Invariant[] = [
  ...
  { id:'CL1a', statement:'The band rule sets form a chain — every rule a looser band requires, a stricter band also requires',
    enforcement:'ci-assertion', owner:'Caitie',
    ciScope:'BAND_SAFETY_RULES is walked strictest-first and any rule required by a looser band but not a stricter one fails the build',
    ...
    validated:'tools/test-gate.mjs breaks the chain and requires the gate to fail',
    test:'a rule in B16_17 but not B13_15 -> build fails',
    source:'doc 33 CL1a' },

  { id:'CL1b', statement:'A card clears every safety rule required by each band it declares',
    enforcement:'ci-assertion', owner:'Caitie',
    ciScope:'for every declared band, BAND_SAFETY_RULES[band] minus card.restrictionsCleared must be empty, over review-gated sources',
    ...
    validated:'tools/test-gate.mjs seeds a card declaring a band whose rules it does not clear and requires the gate to fail',
    test:'a card declaring B13_15 without R_romantic_escalation -> build fails',
    source:'doc 33 CL1b' },

  { id:'CL7', statement:'Every card in the authoritative library carries an APPROVE review record made against the current rule version',
    enforcement:'ci-assertion', owner:'Caitie (the check) \u2014 the decision is Atlas (OWN 2)',
    ciScope:'over review-gated sources: a card with no reviewRecord, a disposition other than APPROVE, a missing reviewer or date, or a rulesVersion other than the current one fails the build',
    ...
    validated:'tools/test-gate.mjs seeds a card with no record, and separately one with a stale rulesVersion, and requires the gate to fail on each',
